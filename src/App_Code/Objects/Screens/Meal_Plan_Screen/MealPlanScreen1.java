...
public class MealPlanScreen1 extends JPanel
{
    private String name = "MyJTable_JDBC11_05";
    //########################################################
    // Objects
    //########################################################
    ...
    private final ArrayList<Integer>
            ...
            ingredientsTable_Hidden_Columns = new ArrayList<Integer>(Arrays.asList(1, 2)), //new Integer[]{0, 0, 0}
            macros_Targets_Table_Hidden_Col = new ArrayList<Integer>(Arrays.asList(1, 2)), //new Integer[]{0, 0, 0}
            macrosLeft_Table_Hidden_Col = new ArrayList<Integer>(Arrays.asList(1, 2)); // new Integer[]{0, 0}; // new Integer[]{0, 0}
...

    {
        ...
        if (planID!=null)
        {    //####################################################
            // Transferring this plans Macros to Temp-Plan
            //####################################################

            String query00 = String.format("""
                    UPDATE `plans` AS `P`,
                    (
                    	SELECT Plan_Name, Vegan FROM Plans WHERE SelectedPlan = 1
                    ) AS `SRC`
                                        
                    SET
                        `P`.`Plan_Name` = concat("(Temp) ",`SRC`.`Plan_Name`),`P`.`Vegan` = `SRC`.`Vegan`
                    WHERE
                        `P`.`PlanID` = %s; """, tempPlanID);
            String query000 = String.format("DELETE FROM macros_Per_Pound_And_Limits WHERE PLANID =%s;", tempPlanID);
            String query01 = String.format("DROP TABLE IF EXISTS temp_Macros;");
            String query02 = String.format(" CREATE table temp_Macros AS SELECT * FROM macros_Per_Pound_And_Limits WHERE PLANID = %s;", planID);
            String query03 = String.format(" ALTER TABLE temp_Macros DROP COLUMN current_Weight_In_Pounds;");
            String query04 = String.format("UPDATE temp_Macros SET PlanID = %s;", tempPlanID);
            ...
            String[] query_Temp_Data = new String[]{query00, query000, query01, query02, query03, query04, query05, query06,
    ...
    //#####################################
    public void macrosTagets_Screen()
    {
        if (!(get_IsPlanSelected()))
        {
            return;
