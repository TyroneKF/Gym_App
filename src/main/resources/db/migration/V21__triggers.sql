...
*/
    ...
    -- #############################################
    -- Prevents Deletes On Table
    -- #############################################
    DELIMITER $$

        CREATE TRIGGER prevent_delete_on_active_user
        BEFORE DELETE ON active_user
        FOR EACH ROW
        BEGIN
          SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'DELETE is not allowed on active_user';
        END$$

    DELIMITER ;

    -- #############################################
    -- Prevents Inserts on Table
    -- #############################################
    DELIMITER $$

            CREATE TRIGGER prevent_insert_on_active_user
            BEFORE INSERT ON active_user
            FOR EACH ROW
            BEGIN
              SIGNAL SQLSTATE '45000'
                 SET MESSAGE_TEXT =
                        'Inserts are not allowed on table active_user';
            END$$
