...
public abstract class MyJTable<T extends Enum<T> & Table_Enum> extends JPanel
{
    //##############################################
    // Objects
    //##############################################
    ...
    (
            ...
            TableMeta<T> table_meta,
            ColumnUiRules<T> column_ui_rules
    )
    {
        ...
        this.column_ui_rules = column_ui_rules;
        ...
        this.column_Names = column_ui_rules.get_Column_Names();
    ...
    private void parent_Table_Configuration()
    {
        ...
        SetUp_Hidden_Table_Columns(); // Hide Columns | Must be the last step in configuration of the table
    ...
    protected void tableModel_Setup(ArrayList<ArrayList<Object>> data, ArrayList<String> column_Names)
    {
        ...
        {
            if (column_ui_rules.is_Columns_To_Hide_Null())
            {
                SetUp_Hidden_Table_Columns();
            }
        }
        else
        {
    ...
    {
        // Variables
        ArrayList<T> col_to_avoid_centering_source = column_ui_rules.get_Col_To_Avoid_Centering();

        if (col_to_avoid_centering_source == null) { return; } // exit

        Set<String> col_to_avoid_centering =
                col_to_avoid_centering_source
                        .stream()
                        .map(Table_Enum :: key)
                        .collect(Collectors.toSet());
        ...
        for (String column_Name : column_Names)
        {
            ...
            if (col_to_avoid_centering.contains(column_Name))
            {
                continue;
            }
    ...
    // Hide Column Methods
    //##########################
    protected void SetUp_Hidden_Table_Columns()
    {
        // Variables
        ArrayList<T> columns_to_hide = column_ui_rules.get_Columns_To_Hide();

        Set<String> hidden_column_names =
                columns_to_hide
                        .stream()
                        .map(Table_Enum :: key)
                        .collect(Collectors.toSet());
        ...
        // Hide Columns
        int no_Of_Columns_Hidden = 0;
        for (int pos = 0; pos < column_Names.size(); pos++)
        {
            // Variables
            int jTable_Pos_After_Hiding = pos - no_Of_Columns_Hidden;
            String column_name = column_Names.get(pos);

            // Hide Column In JTable
            if (hidden_column_names.contains(column_name))
            {
